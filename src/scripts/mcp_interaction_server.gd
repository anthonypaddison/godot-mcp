...
func _ready() -> void:
	...
	_init_key_map()
	_session_token = OS.get_environment("GODOT_MCP_SESSION_TOKEN")
	if _session_token.length() < 32:
		push_error("McpInteractionServer: missing session credential")
		return
	_server = TCPServer.new()
	var err: int = _server.listen(0, "127.0.0.1")
	if err != OK:
		push_error("McpInteractionServer: Failed to listen on loopback, error: %d" % err)
		return
	print("MCP_READY %d" % _server.get_local_port())
...
func _handle_command(json_str: String) -> void:
	...
	if data.get("token", "") != _session_token:
		_send_response_raw({"error": "Authentication failed", "id": req_id})
		return
	...
	if command in ["eval", "http_request", "websocket", "script", "os_info", "call_method", "rpc", "multiplayer"]:
		_busy = false
		_busy_since = 0.0
		_current_id = null
		_send_response({"error": "This command is disabled in the hardened MCP runtime.", "id": req_id})
		return
