...
import { join as pathJoin, dirname, basename, normalize, resolve, relative, isAbsolute } from 'path';
import { existsSync, readdirSync, readFileSync, writeFileSync, copyFileSync, unlinkSync, mkdirSync, renameSync, realpathSync, lstatSync } from 'fs';
import { spawn, execFile } from 'child_process';
import { promisify } from 'util';
import { createConnection, Socket } from 'net';
import { randomBytes } from 'crypto';
import { createRecoveryManifest, hashContent, recoverStaleSessions, removeRecoveryManifest, updateRecoveryManifest, type RecoveryManifest } from './recovery.js';
import {
  canonicalAllowedRoots,
  isContainedPath as isContainedFilesystemPath,
...
const configuredAllowedDirs = process.env.GODOT_MCP_ALLOWED_DIRS || '';
if (!configuredAllowedDirs.trim()) {
  throw new Error('GODOT_MCP_ALLOWED_DIRS is required; refusing to start without an explicit filesystem boundary.');
}

const ALLOWED_PROJECT_ROOTS: string[] = canonicalAllowedRoots(configuredAllowedDirs);

if (ALLOWED_PROJECT_ROOTS.length === 0) {
  throw new Error('GODOT_MCP_ALLOWED_DIRS contains no existing canonical directories.');
}

function isPathWithinAllowedRoots(target: string): boolean {
  if (!target || !isAbsolute(target)) return false;
  try {
    const existing = existsSync(target) ? realpathSync(target) : resolve(target);
    return ALLOWED_PROJECT_ROOTS.some(root => isContainedFilesystemPath(root, existing));
  } catch {
    return false;
  }
}

function canonicalProjectPath(target: string): string | null {
  try { return resolveAllowedProjectRoot(target, ALLOWED_PROJECT_ROOTS); } catch { return null; }
}

function canonicalizeProjectArgs(args: { projectPath: string }): boolean {
  const canonical = canonicalProjectPath(args.projectPath);
  if (!canonical) return false;
  args.projectPath = canonical;
  return true;
}

function projectPathError(path: string): string {
  return `Project path is outside the configured allowed roots or is a symlink: ${path}`;
}

function validateOperationPaths(operation: string, projectPath: string, params: OperationParams): boolean {
  const pathFields = new Set(['scenePath', 'scriptPath', 'resourcePath', 'texturePath', 'filePath', 'outputPath', 'newPath']);
  const writableFields = new Set(['outputPath', 'newPath']);
  if (operation === 'create_scene') writableFields.add('scenePath');
  if (operation === 'create_resource' || (operation === 'manage_theme_resource' && params.action === 'create')) writableFields.add('resourcePath');
  if (operation === 'save_scene' && params.newPath) writableFields.add('newPath');
  if (operation === 'manage_resource' && params.action === 'modify') writableFields.add('resourcePath');
  if (operation === 'create_scene' && typeof params.rootNodeType === 'string' &&
    (params.rootNodeType.startsWith('res://') || /\.gd$/i.test(params.rootNodeType))) {
    if (!isSafeProjectResourcePath(projectPath, params.rootNodeType)) return false;
  }
  const containsUnsafeResourceReference = (value: unknown): boolean => {
    if (typeof value === 'string') return value.startsWith('res://') && !isSafeProjectResourcePath(projectPath, value);
    if (Array.isArray(value)) return value.some(containsUnsafeResourceReference);
    if (value && typeof value === 'object') return Object.values(value).some(containsUnsafeResourceReference);
    return false;
  };
  if (containsUnsafeResourceReference(params.properties)) return false;
  for (const [key, value] of Object.entries(params)) {
    if (!pathFields.has(key) || typeof value !== 'string') continue;
    const resourcePath = value.startsWith('res://') ? value.slice('res://'.length) : value;
    if (!resourcePath || value.startsWith('user://')) return false;
    try {
      if (writableFields.has(key)) resolveWritableProjectPath(projectPath, resourcePath, ALLOWED_PROJECT_ROOTS);
      else resolveExistingProjectPath(projectPath, resourcePath, ALLOWED_PROJECT_ROOTS);
    } catch {
      return false;
    }
  }
...
function isSafeProjectChild(projectPath: string, childPath: string, allowMissing = false): boolean {
  try {
    if (allowMissing) resolveWritableProjectPath(projectPath, childPath, ALLOWED_PROJECT_ROOTS);
    else resolveExistingProjectPath(projectPath, childPath, ALLOWED_PROJECT_ROOTS);
    return true;
  } catch { return false; }
}

function isSafeProjectResourcePath(projectPath: string, resourcePath: string, allowMissing = false): boolean {
  if (resourcePath.startsWith('user://')) return false;
  const childPath = resourcePath.startsWith('res://') ? resourcePath.slice(6) : resourcePath;
  return isSafeProjectChild(projectPath, childPath, allowMissing);
}

function join(first: string, ...rest: string[]): string {
  return secureProjectJoin(first, rest, ALLOWED_PROJECT_ROOTS);
}
...
class GodotServer {
  ...
  private recoveryManifest: RecoveryManifest | null = null;
  ...
  constructor(config?: GodotServerConfig) {
    ...
    this.operationsScriptPath = pathJoin(__dirname, 'scripts', 'godot_operations.gd');
    this.interactionScriptPath = pathJoin(__dirname, 'scripts', 'mcp_interaction_server.gd');
    this.validateScriptPath = pathJoin(__dirname, 'scripts', 'validate_script.gd');
    ...
    const staleConflicts = recoverStaleSessions(projectPath => isPathWithinAllowedRoots(projectPath));
    if (staleConflicts.length > 0) {
      console.error(`[SERVER] Stale MCP recovery conflict; refusing restoration for: ${staleConflicts.join(', ')}`);
    }
  ...
  private injectInteractionServer(projectPath: string): void {
    ...
    const manifest: RecoveryManifest = {
      ...
      projectPath,
      projectFile,
      originalHash: hashContent(existingContent),
      modifiedHash: hashContent(content),
      originalContent: existingContent,
      generatedFiles: [destScript, `${destScript}.uid`],
      sessionId,
      state: 'prepared',
    };
    this.recoveryManifest = manifest;
    createRecoveryManifest(manifest);
    writeFileSync(projectFile, content, 'utf8');
    manifest.state = 'active';
    updateRecoveryManifest(manifest);
    ...
    if (manifest) {
      try {
        ...
        if (currentHash === manifest.modifiedHash) {
          ...
          console.error(`[SERVER] Cleanup conflict; preserving concurrent edit to ${manifest.projectFile}`);
          this.recoveryManifest = null;
          return;
        }
        for (const generated of manifest.generatedFiles) {
          try { if (existsSync(generated)) unlinkSync(generated); } catch (error) {
            console.error(`[SERVER] Failed to remove generated MCP file ${generated}: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
        removeRecoveryManifest(manifest.sessionId);
        this.recoveryManifest = null;
      } catch (error) {
        console.error(`[SERVER] Cleanup restoration failed: ${error instanceof Error ? error.message : String(error)}`);
        return;
      }
    } else {
      // Legacy fallback for a project-managed autoload; only remove the exact known line.
  ...
  private async connectToGame(projectPath: string): Promise<void> {
    ...
    for (let wait = 0; wait < 40 && (!this.gameConnection.interactionPort || !this.gameConnection.sessionToken); wait++) {
      await new Promise(resolve => setTimeout(resolve, 250));
    }
    if (!this.gameConnection.interactionPort || !this.gameConnection.sessionToken) {
      throw new Error('Godot runtime did not announce an authenticated interaction endpoint.');
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          const socket = createConnection({ host: '127.0.0.1', port: this.gameConnection.interactionPort! }, () => {
            ...
            console.error(`[SERVER] Connected to game interaction server on ephemeral loopback port ${this.gameConnection.interactionPort}`);
  ...
  private disconnectFromGame(): void {
    ...
    this.gameConnection.connected = false;
    this.gameConnection.responseBuffer = '';
    this.gameConnection.interactionPort = null;
    this.gameConnection.sessionToken = null;
  ...
  private async sendGameCommand(command: string, params: Record<string, any> = {}, timeoutMs: number = 10000): Promise<any> {
    ...
    const payload = JSON.stringify({ command, params, id, token: this.gameConnection.sessionToken }) + '\n';
  ...
  private async headlessOp(
    ...
    args = normalizeParameters(args || {});
    const { projectPath: requestedProjectPath, params } = argsFn(args);

    if (!requestedProjectPath) return createErrorResponse('projectPath is required.');
    const projectPath = canonicalProjectPath(requestedProjectPath);
    if (!projectPath) return createErrorResponse(projectPathError(requestedProjectPath));
    if (!validateOperationPaths(operation, projectPath, params)) return createErrorResponse('Operation path is outside the configured project root or is a symlink.');
        ...
        },
{
      ...
      ].filter(tool => !REMOVED_TOOLS.has(tool.name)),
    }));

    // Handle tool calls
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      this.logDebug(`Handling tool request: ${request.params.name}`);
      if (REMOVED_TOOLS.has(request.params.name)) {
        return createErrorResponse(`Tool ${request.params.name} is disabled in the hardened MCP.`);
      }
  ...
  private async handleLaunchEditor(args: any) {
    ...
    if (!canonicalizeProjectArgs(args)) {
      return createErrorResponse(
        'Invalid project path'
      );
    }
    if (args.scene && (!validatePath(args.scene) || !isSafeProjectResourcePath(args.projectPath, args.scene))) {
      return createErrorResponse('Scene path is outside the configured project root or is a symlink.');
    }
  ...
  private async handleRunProject(args: any) {
    ...
    try {
      ...
      // Create a fresh per-session credential. It is passed only to the disposable Godot process.
      this.gameConnection.sessionToken = randomBytes(32).toString('hex');
      this.gameConnection.interactionPort = null;
      ...
      const process = spawn(this.godotPath!, cmdArgs, {
        stdio: 'pipe',
        env: { ...globalThis.process.env, GODOT_MCP_SESSION_TOKEN: this.gameConnection.sessionToken },
      });
      const output: string[] = [];
      const errors: string[] = [];
      ...
      process.stdout?.on('data', (data: Buffer) => {
        ...
        lines.forEach((line: string) => {
          const ready = line.match(/^MCP_READY (\d+)$/);
          if (ready) this.gameConnection.interactionPort = Number(ready[1]);
      ...
      this.connectToGame(args.projectPath).catch(err => {
        console.error(`[SERVER] Failed to connect to game interaction server: ${err instanceof Error ? err.message : String(err)}`);
        this.cleanupFailedRuntime(process);
      });
      ...
      return {
        content: [
          {
            type: 'text',
            text: 'Godot project started in debug mode. Use get_debug_output to see output. Connecting to an authenticated ephemeral loopback interaction server...',
          },
        ],
      };
    } catch (error: unknown) {
  ...
  private cleanupFailedRuntime(process: any): void {
    this.disconnectFromGame();
    if (this.gameConnection.projectPath) {
      this.removeInteractionServer(this.gameConnection.projectPath);
      this.gameConnection.projectPath = null;
  ...
  private async handleListProjects(args: any) {
    ...
    const allowedDirectory = canonicalProjectPath(args.directory);
    if (!allowedDirectory) return createErrorResponse('Directory is outside the configured allowed roots or is a symlink.');
    args.directory = allowedDirectory;
  ...
  private async handleGetProjectInfo(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args)) {
      return createErrorResponse(
        'Invalid project path'
      );
    }
  ...
  private async handleCreateScene(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args) || !validatePath(args.scenePath)) {
      return createErrorResponse(
        'Invalid path'
      );
    }

    if (!isSafeProjectResourcePath(args.projectPath, args.scenePath, true)) {
      return createErrorResponse('Scene output path is outside the configured project root or is a symlink.');
    }
  ...
  private async handleLoadSprite(args: any) {
    ...
    if (
      !validatePath(args.projectPath) || !canonicalizeProjectArgs(args) ||
    ...
    try {
      const outputPath = args.outputPath.startsWith('res://') ? args.outputPath.slice(6) : args.outputPath;
      resolveSafeOutputPath(args.projectPath, outputPath, ALLOWED_PROJECT_ROOTS);
    } catch {
      return createErrorResponse('Output path is outside the configured project root or is a symlink.');
    }
  ...
  private async handleExportMeshLibrary(args: any) {
    ...
    if (
      !validatePath(args.projectPath) || !canonicalizeProjectArgs(args) ||
      !validatePath(args.scenePath) ||
      !validatePath(args.outputPath) || args.outputPath.startsWith('user://')
    ) {
      return createErrorResponse(
        'Invalid path'
      );
    ...
    let outputResourcePath: string;
    try {
      const projectRoot = args.projectPath;
      const outputPath = args.outputPath.startsWith('res://')
        ? args.outputPath.slice('res://'.length)
        : args.outputPath;
      let outputChildPath = outputPath;
      if (isAbsolute(outputPath)) {
        const absoluteOutput = resolve(outputPath);
        let existingParent = dirname(absoluteOutput);
        while (!existsSync(existingParent)) {
          const parent = dirname(existingParent);
          if (parent === existingParent) throw new Error('Output path has no existing parent');
          existingParent = parent;
        }
        const canonicalParent = realpathSync(existingParent);
        const canonicalOutput = pathJoin(canonicalParent, relative(existingParent, absoluteOutput));
        outputChildPath = relative(projectRoot, canonicalOutput);
        if (!outputChildPath || !isContainedFilesystemPath(projectRoot, canonicalOutput)) {
          return createErrorResponse('Invalid path');
        }
      }
      const resolvedOutputPath = resolveWritableProjectPath(
        projectRoot,
        outputChildPath,
        ALLOWED_PROJECT_ROOTS
      );
      const resourcePath = relative(projectRoot, resolvedOutputPath).replace(/\\/g, '/');
      if (!resourcePath || resourcePath === '..' || resourcePath.startsWith('../')) {
        return createErrorResponse('Invalid path');
      }
      outputResourcePath = `res://${resourcePath}`;
    } catch {
      return createErrorResponse('Invalid path');
    }
  ...
  private async handleSaveScene(args: any) {
    ...
    if (args.newPath && !isSafeProjectResourcePath(args.projectPath, args.newPath, true)) {
      return createErrorResponse('New scene path is outside the configured project root or is a symlink.');
    }
  ...
  private async handleGetUid(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args) || !validatePath(args.filePath)) {
      return createErrorResponse(
        'Invalid path'
      );
    }
  ...
  private async handleCreateDirectory(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args) || !validatePath(args.directoryPath))
      return createErrorResponse('Invalid path.');
    const projectFile = join(args.projectPath, 'project.godot');
    if (!existsSync(projectFile))
      return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
  ...
  private async handleCreateProject(args: any) {
    ...
    if (!validatePath(args.projectPath) || isAbsolute(args.projectPath))
      return createErrorResponse('Invalid path.');
    try {
      const creationRoot = ALLOWED_PROJECT_ROOTS.find(root => isContainedFilesystemPath(root, resolve(args.projectPath)));
      if (!creationRoot) return createErrorResponse('Project path is outside the allowed roots.');
      const safeProjectPath = resolveWritableProjectPath(
        creationRoot,
        relative(creationRoot, resolve(args.projectPath)),
        ALLOWED_PROJECT_ROOTS,
      );
      if (!existsSync(safeProjectPath)) {
        mkdirSync(safeProjectPath, { recursive: true });
      }
      const projectFile = join(safeProjectPath, 'project.godot');
      if (existsSync(projectFile))
      ...
      if (isDotnet) {
        ...
        writeFileSync(join(safeProjectPath, `${assemblyName}.csproj`), generateCsprojContent(args.projectName, sdkVersion), 'utf8');
      }
  ...
  private async handleCreateCsharpScript(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args) || !validatePath(args.scriptPath)) return createErrorResponse('Invalid path.');
    const projectFile = join(args.projectPath, 'project.godot');
  ...
  private async handleExportProject(args: any) {
    ...
    const outputChild = args.outputPath.startsWith('res://') ? args.outputPath.slice(6) : args.outputPath;
    let safeOutputPath: string;
    try {
      safeOutputPath = resolveSafeOutputPath(args.projectPath, outputChild, ALLOWED_PROJECT_ROOTS);
    } catch {
      return createErrorResponse('Export output path is outside the configured project root or is a symlink.');
    }
    const projectFile = join(args.projectPath, 'project.godot');
    if (!existsSync(projectFile))
      return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
    ...
    try {
      ...
      const exportArgs = ['--headless', '--path', args.projectPath, exportFlag, args.presetName, safeOutputPath];
  ...
  private async handleRenameFile(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args) || !validatePath(args.filePath) || !validatePath(args.newPath)) return createErrorResponse('Invalid path.');
    const projectFile = join(args.projectPath, 'project.godot');
    if (!existsSync(projectFile)) return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
  ...
  private async handleValidateScripts(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args)) return createErrorResponse('Invalid path.');
    const projectFile = join(args.projectPath, 'project.godot');
    if (!existsSync(projectFile)) return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
    if (!this.godotPath) {
      await this.detectGodotPath();
  ...
  private async handleManageShader(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args) || !validatePath(args.shaderPath)) return createErrorResponse('Invalid path.');
    const projectFile = join(args.projectPath, 'project.godot');
    if (!existsSync(projectFile)) return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
  ...
  private async handleSetMainScene(args: any) {
    ...
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args)) return createErrorResponse('Invalid path.');
    if (!isSafeProjectResourcePath(args.projectPath, args.scenePath)) return createErrorResponse('Scene path is outside the configured project root or is a symlink.');
    const projectFile = join(args.projectPath, 'project.godot');
    if (!existsSync(projectFile)) return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
    try {
      let content = readFileSync(projectFile, 'utf8');
  ...
  private async handleManageTranslations(args: any) {
    ...
    if (!args.projectPath || !args.action) return createErrorResponse('projectPath and action are required.');
    if (!validatePath(args.projectPath) || !canonicalizeProjectArgs(args)) return createErrorResponse('Invalid path.');
    if (args.action === 'add' && (!args.translationPath || !isSafeProjectResourcePath(args.projectPath, args.translationPath, true))) {
      return createErrorResponse('Translation path is outside the configured project root or is a symlink.');
    }
    const projectFile = join(args.projectPath, 'project.godot');
    if (!existsSync(projectFile)) return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
    try {
  ...
  private async handleManageCiPipeline(args: any) {
    ...
    const workflowPath = pathJoin(workflowDir, 'godot-export.yml');
    try {
      if (args.action === 'read') {
