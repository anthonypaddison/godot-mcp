...
import { canonicalAllowedRoots, secureProjectJoin } from '../src/filesystem-security.js';

const __dirname = dirname(fileURLToPath(import.meta.url));

const boundaryCleanups: string[] = [];
afterEach(() => { while (boundaryCleanups.length) rmSync(boundaryCleanups.pop()!, { recursive: true, force: true }); });

function boundaryFixture() {
  const base = mkdtempSync(join(tmpdir(), 'godot-mcp-boundary-'));
  const allowed = join(base, 'allowed');
  const project = join(allowed, 'game');
  const outside = join(base, 'outside');
  mkdirSync(project, { recursive: true });
  mkdirSync(outside);
  writeFileSync(join(project, 'project.godot'), '[application]\\nconfig/name="Fixture"\\n');
  writeFileSync(join(project, 'main.tscn'), '[gd_scene]\\n');
  writeFileSync(join(project, 'safe.gdshader'), 'shader_type spatial;');
  writeFileSync(join(outside, 'sentinel.txt'), 'unchanged');
  boundaryCleanups.push(base);
  return { base, allowed, project, outside, roots: canonicalAllowedRoots(allowed) };
}
...
function fakeHeadlessOp(
  ...
  if (!projectPath.startsWith('/') || !isContainedPath('/tmp/allowed', projectPath)) return { error: 'Invalid path.', commandArgs: null } as any;
...
describe('Headless operation handlers — argument transforms', () => {
  describe('handleModifySceneNode', () => {
    ...
    it('maps all params correctly', () => {
      const r = fakeHeadlessOp({
        projectPath: '/tmp/allowed/game',
        scenePath: 'scenes/main.tscn',
        nodePath: '/root/Player',
        properties: { visible: true },
      }, argsFn);
      expect(r.error).toBeNull();
      expect(r.operation!.projectPath).toBe('/tmp/allowed/game');
  ...
  describe('handleRemoveSceneNode', () => {
    ...
    it('maps params', () => {
      const r = fakeHeadlessOp({
        projectPath: '/tmp/allowed/game', scenePath: 'main.tscn', nodePath: '/root/Enemy',
      }, argsFn);
      expect(r.error).toBeNull();
  ...
  describe('handleAttachScript', () => {
    ...
    it('maps all params', () => {
      const r = fakeHeadlessOp({
        projectPath: '/tmp/allowed/game', scenePath: 'main.tscn',
  ...
  describe('handleCreateResource', () => {
    ...
    it('maps required params', () => {
      const r = fakeHeadlessOp({
        projectPath: '/tmp/allowed/game', resourceType: 'PackedScene', resourcePath: 'res://new.tres',
      }, argsFn);
      expect(r.error).toBeNull();
    ...
    it('includes optional properties', () => {
      const r = fakeHeadlessOp({
        projectPath: '/tmp/allowed/game', resourceType: 'Theme', resourcePath: 'res://theme.tres',
...
describe('headlessOp path validation', () => {
  ...
  it('accepts valid path when project exists', () => {
    const r = fakeHeadlessOp({ projectPath: '/tmp/allowed/game' }, simpleArgsFn, true);
    expect(r.error).toBeNull();
  });

  it('rejects when project does not exist', () => {
    const r = fakeHeadlessOp({ projectPath: '/tmp/allowed/game' }, simpleArgsFn, false);
    expect(r.error).toContain('Not a valid Godot project');
  });
});

describe('exposed handler boundary regressions', () => {
  it.each([
    ['get_project_info', 'GetProjectInfo'], ['manage_layers', 'ManageLayers'], ['manage_shader', 'ManageShader'],
    ['read_file', 'ReadFile'], ['write_file', 'WriteFile'], ['export_project', 'ExportProject'],
  ])('%s uses fail-closed project joining', (tool, method) => {
    const f = boundaryFixture();
    const expected = `${f.roots[0]}/game/project.godot`;
    expect(secureProjectJoin(f.project, ['project.godot'], f.roots)).toBe(expected);
    expect(() => secureProjectJoin(f.outside, ['project.godot'], f.roots)).toThrow();
    expect(readFileSync(join(f.outside, 'sentinel.txt'), 'utf8')).toBe('unchanged');
    const handlerStart = sourceCode.indexOf(`private async handle${method}`);
    expect(handlerStart).toBeGreaterThanOrEqual(0);
    const handlerEnd = sourceCode.indexOf('\n  private ', handlerStart + 1);
    expect(sourceCode.slice(handlerStart, handlerEnd > 0 ? handlerEnd : undefined)).toContain('canonicalizeProjectArgs');
  });

  it.each(['../outside/sentinel.txt', '../../outside/sentinel.txt', '/etc/passwd'])('rejects secondary filesystem input %s', child => {
    const f = boundaryFixture();
    const relativeChild = child.startsWith('../') ? `../../${child.slice(3)}` : child;
    expect(() => secureProjectJoin(f.project, [relativeChild], f.roots)).toThrow();
  });

  it('rejects project-root and child symlink escapes before handler I/O', () => {
    const f = boundaryFixture();
    symlinkSync(f.project, join(f.allowed, 'project-link'));
    symlinkSync(join(f.outside, 'sentinel.txt'), join(f.project, 'file-link'));
    expect(() => secureProjectJoin(join(f.allowed, 'project-link'), ['project.godot'], f.roots)).toThrow();
    expect(() => secureProjectJoin(f.project, ['file-link'], f.roots)).toThrow();
  });

  it('blocks outside-root reads, writes, scans, and process callbacks before they run', () => {
    const f = boundaryFixture();
    let reads = 0;
    let writes = 0;
    let scans = 0;
    let processes = 0;
    expect(() => {
      const target = secureProjectJoin(f.outside, ['sentinel.txt'], f.roots);
      reads++;
      return readFileSync(target, 'utf8');
    }).toThrow();
    expect(() => {
      const target = secureProjectJoin(f.project, ['../../outside/new.txt'], f.roots);
      writes++;
      writeFileSync(target, 'bad');
    }).toThrow();
    expect(() => {
      const target = secureProjectJoin(f.outside, [], f.roots);
      scans++;
      return existsSync(target);
    }).toThrow();
    expect(() => {
      secureProjectJoin(f.outside, ['project.godot'], f.roots);
      processes++;
    }).toThrow();
    expect({ reads, writes, scans, processes }).toEqual({ reads: 0, writes: 0, scans: 0, processes: 0 });
    expect(existsSync(join(f.outside, 'new.txt'))).toBe(false);
  });

  it('validates list_projects and list_project_files discovery boundaries', () => {
    for (const handler of ['handleListProjects', 'handleListProjectFiles']) {
      const start = sourceCode.indexOf(`private async ${handler}`);
      expect(start).toBeGreaterThanOrEqual(0);
      const end = sourceCode.indexOf('\n  private ', start + 1);
      expect(sourceCode.slice(start, end > 0 ? end : undefined)).toMatch(/canonical(ProjectPath|izeProjectArgs)/);
    }
  });

  it('validates filesystem-valued nested headless parameters before executeOperation', () => {
    const f = boundaryFixture();
    expect(() => secureProjectJoin(f.project, ['../../outside/nested.tres'], f.roots)).toThrow();
    const start = sourceCode.indexOf('private async headlessOp(');
    const end = sourceCode.indexOf('private async executeOperation(', start);
    expect(sourceCode.slice(start, end)).toContain('validateOperationPaths');
  });

  it('uses existing canonical helpers for writable destinations and preserves resource/node references', () => {
    expect(sourceCode).toContain('resolveWritableProjectPath');
    expect(sourceCode).toContain('resolveSafeOutputPath');
    expect(sourceCode).toContain('validateOperationPaths(operation, projectPath, params)');
    expect(sourceCode).toContain('isSafeProjectResourcePath(projectPath, params.rootNodeType)');
    expect(normalizeParameters({ scene_path: 'res://levels/main.tscn', node_path: '/root/Player' })).toEqual({
      scenePath: 'res://levels/main.tscn', nodePath: '/root/Player',
    });
  });
});
...
describe('Handler source structure', () => {
  ...
  it('headlessOp validates projectPath and checks project.godot', () => {
    expect(sourceCode).toContain("if (!requestedProjectPath) return createErrorResponse('projectPath is required.");
    expect(sourceCode).toContain('canonicalProjectPath(requestedProjectPath)');
