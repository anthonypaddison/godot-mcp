...
describe('isContainedPath', () => {
  it('accepts a path inside the root', () => {
    expect(isContainedPath('/tmp/projects/game', '/tmp/projects/game/scripts/main.gd')).toBe(true);
  });

  it('rejects similar-prefix escapes', () => {
    expect(isContainedPath('/tmp/projects/game', '/tmp/projects/game-other/file.txt')).toBe(false);
  });

  it('rejects traversal after resolution', () => {
    expect(isContainedPath('/tmp/projects/game', '/tmp/projects/game/../secrets')).toBe(false);
  });
});
